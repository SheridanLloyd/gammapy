...
from datasets.ogip_spectrum_dataset import StandardOGIPDataset
...
class StandardOGIPDatasetReader:
    ...
    def _read_regions(self, hdulist):
        ...
        if self.region_hdu in hdulist:
            ...
            #wcs = WcsGeom.from_header(region_table.meta).wcs # no NAXIS1 or NAXIS2 in geometry ?
            ...
            #     regions.append(reg.to_sky(wcs))
            # region = list_to_compound_region(regions)
            # region=pix_region.to_sky(wcs) # SJL - deal with regions that are simple spectral without a component
            region = pix_region[0] # its a list with one thing in it
        #return region, wcs
        return region
    ...
    def read(self, filenames=None, name=None):
        ...
        #region, wcs = self._read_regions(hdulist) # dont need geometry for a point source spectrum
        region = self._read_regions(hdulist)
        ...
        #geom = RegionGeom(region=region, wcs=wcs, axes=[energy_axis]) # no WCS for spectrum
        ...
        geom_true = RegionGeom(region=None, axes=[energy_true_axis])
        #geom_true = RegionGeom(region=region, axes=[energy_true_axis]) # fails pix coord object has no frame
        #geom_true = RegionGeom(region=region, wcs=wcs, axes=[energy_true_axis]) #original
